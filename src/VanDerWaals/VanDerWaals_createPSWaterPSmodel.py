...
parameterSweep = pd.read_csv('VanDerWaals_parameterSweep.csv', delimiter = ',')
...
dump(my_interpolating_function, 'VanDerWaals_Model_PSWaterPS.joblib') 
my_interpolating_function= load('VanDerWaals_Model_PSWaterPS.joblib')
...
fig, ax = plt.subplots()
ax.plot(h0s*1e9,dataShow,'x',color='dodgerblue', markersize=4, label='Calculated, C = 10^{-5} M')
...
ax.plot(h0s*1e9,dataShow2,'x',color='darkviolet', markersize=4, label='Calculated, C = 0.16 M')
...
print(C0s[idC])
dataShow2 = [data[idT][idC][k] for k in range(len(h0s))]
ax.plot(h0s*1e9,dataShow2,'x',color='crimson', markersize=4, label='Calculated, C = 10^{3} M')
...
ax.set_ylabel('Hamaker Constant $A_{PWP}(h)/k_B T$')
plt.xscale('log')
...
ax.set_ylim(0,3.5)
plt.tight_layout()
plt.savefig('VanDerWaals_PWP_HeightPlots.pdf')
plt.show()
#
