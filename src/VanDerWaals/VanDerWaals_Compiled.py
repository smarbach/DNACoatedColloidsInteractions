...
def integrandVdW(x,xin,h):
    return x*(log(1 - DeltaWS(x,xin,h)**2*exp(-x))+ log(1 - DeltabarWS(x,xin,h)**2*exp(-x)))
...
def hamaker(lambdaD,T,h):
        ...
        for n in range(200): # accuracy at 3rd digit
            ...
            if n==0:
                ...
                while abs(avalue) > 0 :
                    rmax = rmax*2
                    avalue = integrandVdW(rmax,xinv,h) # this is to make sure the integration will go as far as needed. 
...
for Tcelsius in Ts:
    for c0s in Cs:
        ...
        for i in range(len(h0s)):
            ...
            f= open("VanDerWaals_parameterSweep.txt","a+")
