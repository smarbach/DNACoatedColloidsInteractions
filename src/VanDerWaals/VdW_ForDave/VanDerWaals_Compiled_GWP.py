...
This code calculates the Hamaker constant for the glass - water - PS system
at various heights, temperatures and salt concentrations
"""
...
## 1 - SET PARAMETER RANGE

h0s = np.logspace(-10,-5.5,100) # heights
Ts = np.linspace(20,100,20) # Temperatures
Cs = np.logspace(-5,3,20) # salt concentrations (in mol/L)


## 2 - COMPILED FUNCTIONS TO CALCULATE THE HAMAKER FUNCTION

#@numba.njit(float32(float32))
#def epsilonW(xin: float) -> float:
@jit(float32(float32),nopython=True,cache=True)
def epsilonW(xin):    #dielectric permittivity of water
...
@jit(float32(float32),nopython=True,cache=True)
def epsilonG(xin): #dielectric permittivity of glass
...
@jit(float32(float32),nopython=True,cache=True)
def epsilonPS(xin): #dielectric permittivity of polystyrene
...
def DeltaWG(x,xin,h):
    if x ==0:
        ...
        if xin == 0:
            ratio = (epsilonW(xin) - epsilonG(xin))/(epsilonW(xin) + epsilonG(xin))
        else:
            ratio = (epsilonW(xin)*sG(x,xin,h) - epsilonG(xin)*sW(x))/(epsilonW(xin)*sG(x,xin,h) + epsilonG(xin)*sW(x))
    return ratio
...
def DeltabarWG(x,xin,h):
    if x ==0:
        ...
        if xin == 0:
            ...
            ratio = (sG(x,xin,h) - sW(x))/(sG(x,xin,h) + sW(x))
    return ratio
    
#@numba.njit(float32(float32, float32, float32))
#def integrandVdW(x: float,xin: float, h:float) -> float:
@jit(float32(float32, float32, float32),cache=True,nopython=True)
def integrandVdW(x,xin,h):
    return x*(log(1 - DeltaWS(x,xin,h)*DeltaWG(x,xin,h)*exp(-x))+ log(1 - DeltabarWS(x,xin,h)*DeltabarWG(x,xin,h)*exp(-x)))


@jit(float32(float32, float32, float32),cache=True)
def hamaker(lambdaD,T,h): # HAMAKER constant
        hbar = 1.054571817*10**(-34)
        kB = 1.380658*10**-23
        mpi = 3.14159265359
        A123 = 0 #hamaker constant 
        # now we are going to sum on all modes 
        for n in range(1000): #I checked that 1000 was sufficient to get enough accuracy for most parameters; 
            if n==0:
            ...
            if n==0:
                rmax = 100 #for n = 0 this is more than enough
            else:
                rmax = 10*rn(xinv,h)
                avalue = integrandVdW(rmax,xinv,h)
                while abs(avalue) > 0 :
                    rmax = rmax*2
                    avalue = integrandVdW(rmax,xinv,h) # this is to make sure the integration will go as far as needed without adding spurious effects
            ...
            a123n,err = quad(integrandVdW,rn(xinv,h),rmax,args=(xinv,h))
...
## 3 - NOW DO THE MAIN LOOP ON PARAMETERS
...
for Tcelsius in Ts:
    for c0s in [0.1]: #Cs:
        ...
        def density(T) :
            # actual density of water is interpolated between data points collected by Jeana
            Tc = []
            densities = []
        ...
        def permittivity(T,rhom) :
            # actual permittivity is calculated from tabulated model (see overleaf)
        ...
        for i in range(len(h0s)):
            Fvalue = hamaker(lambdaD,T,h0s[i])
            print(Fvalue)
            print(h0s[i])
            f= open("VanDerWaals_parameterSweep_GlassWaterPS.txt","a+")
